...
- [Python3. Памятка для начинающих](#python3-памятка-для-начинающих)
  ...
  - [Специальные свойства и методы](#специальные-свойства-и-методы)
    - [Специальные переменные](#специальные-переменные)
    - [Информация о классе](#информация-о-классе)
    - [Информация об объекте](#информация-об-объекте)
    - [Явное приведение объекта к определенному типу](#явное-приведение-объекта-к-определенному-типу)
    - [Представление объекта](#представление-объекта)
    - [Перегрузка операторов](#перегрузка-операторов)
    - [Перегрузка побитовых операторов](#перегрузка-побитовых-операторов)
    - [Контейнеры](#контейнеры)
    - [Доступ к атрибутам](#доступ-к-атрибутам)
    - [Итератор](#итератор)
...
| `__package__`     | Имя пакета, которому принадлежит модуль                           |
| `__dict__`        | Пространство имен модуля в виде словаря                           |
| `__doc__`         | Строка документации (docstring) модуля                            |
| `__annotations__` | Словарь с аннотациями переменных модуля                           |
| `__all__`         | Список имен, экспортируемых при выполнении `from module import *` |

### Информация о классе

| Атрибут           | Описание                                    |
| ----------------- | ------------------------------------------- |
| `__name__`        | Имя класса, функции/метода                  |
| `__qualname__`    | Полное имя класса, функции/метода (>= 3.3)  |
| `__module__`      | Имя модуля                                  |
| `__class__`       | Объект-метакласс (по умолчанию `type`)      |
| `__bases__`       | Кортеж базовых классов                      |
| `__mro__`         | Кортеж классов в порядке разрешения методов |
| `__dict__`        | Словарь атрибутов класса                    |
| `__doc__`         | Строка документации                         |
| `__annotations__` | Аннотации атрибутов класса                  |

### Информация об объекте

| Атрибут      | Описание                                                                                                     |
| ------------ | ------------------------------------------------------------------------------------------------------------ |
| `__class__`  | Класс, экземпляром которого является объект                                                                  |
| `__dict__`   | Словарь атрибутов объекта                                                                                    |
| `__doc__`    | Строка документации                                                                                          |
| `__dir__()`  | Список имен атрибутов объекта, используется функцией `dir()`                                                 |

### Явное приведение объекта к определенному типу

| Атрибут         | Описание                                                                                                      |
| --------------- | ------------------------------------------------------------------------------------------------------------- |
| `__bool__()`    | Приведение к логическому типу `bool()`                                                                        |
| `__int__()`     | Приведение к целочисленному типу `int()`                                                                      |
| `__float__()`   | Приведение к вещественному типу `float()`                                                                     |
| `__complex__()` | Приведение к комплексному числу `complex()`                                                                   |
| `__str__()`     | Приведение к строковому типу `str()`                                                                          |
| `__bytes__()`   | Приведение к байтовой строке `bytes()`                                                                        |
| `__index__()`   | Приведение к целочисленному типу без потерь; используется в индексах, срезах и для перевода числа в другую СС |

### Представление объекта

| Атрибут                   | Описание                                               |
| ------------------------- | ------------------------------------------------------ |
| `__repr__()`              | Каноническое строковое представление объекта: `repr()` |
| `__str__()`               | Строковое представление объекта: `str()`, `print()`    |
| `__format__(format_spec)` | Форматирование через `format()` и f-строки             |

### Перегрузка операторов

Перегрузка операторов с левой ассоциативностью:

| Атрибут               | Оператор    | Пример           |
| --------------------- | ----------- | ---------------- |
| `__add__(other)`      | `+`         | `x + other`      |
| `__sub__(other)`      | `-`         | `x - other`      |
| `__mul__(other)`      | `*`         | `x * other`      |
| `__truediv__(other)`  | `/`         | `x / other`      |
| `__floordiv__(other)` | `//`        | `x // other`     |
| `__mod__(other)`      | `%`         | `x % other`      |
| `__pow__(other)`      | `**`        | `x ** other`     |
| `__matmul__(other)`   | `@`         | `matrix @ other` |
| `__neg__()`           | унарный `-` | `-x`             |
| `__pos__()`           | унарный `+` | `+x`             |
| `__eq__(other)`       | `==`        | `x == other`     |
| `__ne__(other)`       | `!=`        | `x != other`     |
| `__lt__(other)`       | `<`         | `x < other`      |
| `__le__(other)`       | `<=`        | `x <= other`     |
| `__gt__(other)`       | `>`         | `x > other`      |
| `__ge__(other)`       | `>=`        | `x >= other`     |

Для перегрузки операторов с правой ассоциативностью
нужно добавить `r` в начале имени метода: `__radd__`, `__rsub__` и т.п.

Для перегрузки операторов с присваиванием
нужно добавить `i` в начале имени метода: `__iadd__`, `__isub__` и т.п.

### Перегрузка побитовых операторов

| Атрибут             | Оператор | Пример       |
| ------------------- | -------- | ------------ |
| `__and__(other)`    | `&`      | `x & other`  |
| `__or__(other)`     | `\|`     | `x \| other` |
| `__xor__(other)`    | `^`      | `x ^ other`  |
| `__invert__()`      | `~`      | `~x`         |
| `__lshift__(other)` | `<<`     | `x << other` |
| `__rshift__(other)` | `>>`     | `x >> other` |

Для перегрузки операторов с правой ассоциативностью
нужно добавить `r` в начале имени метода: `__rand__`, `__ror__` и т.п.

Для перегрузки операторов с присваиванием
нужно добавить `i` в начале имени метода: `__iand__`, `__ior__` и т.п.

### Контейнеры

| Атрибут                   | Описание                                                      | Пример            |
|---------------------------|---------------------------------------------------------------|-------------------|
| `__delitem__(item)`       | Удаляет элемент с указанным ключом или индексом               | `del x[item]`     |
| `__getitem__(item)`       | Возвращает элемент с указанным ключом или индексом            | `x[item]`         |
| `__setitem__(key, value)` | Присваивает значение элементу с указанным ключом или индексом | `x[key] = value`  |
| `__contains__(item)`      | Проверяет наличие элемента в объекте                          | `item in x`       |
| `__len__()`               | Возвращает количество элементов в объекте                     | `len(x)`          |

### Доступ к атрибутам

| Атрибут                   | Описание                                                           | Пример                           |
|---------------------------|--------------------------------------------------------------------|----------------------------------|
| `__delattr__(item)`       | Удаляет атрибут с указанным именем                                 | `del x.item`                     |
| `__getattr__(item)`       | Вызывается при обращении к несуществующему атрибуту                | `x.item`                         |
| `__getattribute__(item)`  | Вызывается при обращении к любому атрибуту существующему или нет   | `x.item`                         |
| `__setattr__(key, value)` | Присваивает значение атрибуту с указанным именем                   | `x.key = value`                  |
| `__slots__`               | Задаёт фиксированный набор атрибутов и ограничивает создание новых | `__slots__ = ('attr1', 'attr2')` |

### Итератор

| Атрибут          | Описание                                                                                                                                  |
| -----------------| ----------------------------------------------------------------------------------------------------------------------------------------- |
| `__iter__()`     | Возвращает новый объект-итератор, который может перебирать все объекты в контейнере, для сопоставлений должен перебирать ключи контейнера |
| `__next__()`     | Возвращает следующий элемент из итератора                                                                                                 |
| `__reversed__()` | Возвращает итератор для обхода объекта в обратном порядке                                                                                 |
...
| `__exit__(err_type, err, tb)` | Вызывается при выходе из блока `with`, независимо от того, завершился блок успешно или возникло исключение |

| Метод          | Описание                                                      | Пример         |
|----------------|---------------------------------------------------------------|----------------|
| `__abs__()`    | Возвращает модуль (абсолютное значение) числа                 | `abs(x)`       |
| `__round__(n)` | Возвращает число, округлённое до указанного количества знаков | `round(x, n)`  |
| `__trunc__()`  | Возвращает целую часть числа, отбрасывая дробную              | `math.trunc(x)`|
| `__floor__()`  | Возвращает наибольшее целое число, не превышающее значение    | `math.floor(x)`|
| `__ceil__()`   | Возвращает наименьшее целое число, не меньшее значения        | `math.ceil(x)` |
